...
{
    DDXMLNode* child=[GDUHelper getAttribute:node name:name];
    return child!=nil;
}

+(BOOL) hasValue:(DDXMLElement*) node name:(NSString*)name
{
    DDXMLElement* child=[GDUHelper getNode:node name:name];
    if(child!=nil)
    {
        DDXMLNode* nilAttr=[GDUHelper getAttribute:child name:@"nil" url:XSI];
        return nilAttr==nil;
    }
    return NO;
}
